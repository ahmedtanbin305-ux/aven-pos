...
with tab1:
    ...
    if data["sales"]:
        # টেবিল আকারে সেলস দেখানো
        st.table(data["sales"][::-1]) # নতুন বিক্রিগুলো ওপরে দেখাবে
    else:
...
with tab2:
    ...
    with st.form("add_product_form"):
        ...
        if submit_btn:
            if p_name:
                # আগে থেকে থাকলে স্টক যোগ হবে, না থাকলে নতুন তৈরি হবে
                if p_name in data["products"]:
                    data["products"][p_name]["stock"] += p_stock
                    data["products"][p_name]["price"] = p_price # নতুন প্রাইস আপডেট
                else:
    ...
    if data["products"]:
        # সুন্দর ফরম্যাটে ইনভেন্টরি দেখানো
...
with tab3:
        ...
        if data["products"]:
            ...
            with c_update:
                if st.button("🔄 তথ্য আপডেট করুন", use_container_width=True):
                    # নাম যদি চেঞ্জ করে, তবে আগের নাম ডিলিট করে নতুন নামে সেভ হবে
    ...
    # ২. বিক্রির হিসাব এডিট/ডিলিট
    elif edit_choice == "বিক্রির হিসাব":
        if data["sales"]:
            # ড্রপডাউনে দেখানোর জন্য লিস্ট তৈরি
            ...
            # নতুন দাম ক্যালকুলেশন (প্রোডাক্টের বর্তমান রেট অনুযায়ী)
            ...
            with c_s_delete:
                if st.button("🗑️ এই বিক্রির এন্ট্রি ডিলিট করুন", key="del_sale", use_container_width=True):
                    # ডিলিট করার পর প্রোডাক্টের স্টক আবার আগের জায়গায় ফেরত পাঠানো (ঐচ্ছিক কিন্তু ভালো প্র্যাকটিস)
